...
// niuxuewei comments: 做一些无关痛痒的准备工作
func (q *qemu) setup(ctx context.Context, id string, hypervisorConfig *HypervisorConfig) error {
	...
	// niuxuewei comments: initrd 和 image 只要有一个就行
...
// niuxuewei comments: qemu machine 是 qemu 针对 machine 的配置，可以选择 q35 之
// 类的东西，同时还包含一些其他的配置。
...
// niuxuewei comments: 主要就是生成了一个 qemuConfig 结构体
func (q *qemu) CreateVM(ctx context.Context, id string, network Network, hypervisorConfig *HypervisorConfig) error {
	...
	// niuxuewei comments: 这个看起来是跟 SharedFS 相关的，暂略
	...
	// niuxuewei comments: vhost-user store
	// TODO(justxuewei): 这个在使用 DPDK 的时候需要我们考虑么？
	...
	// niuxuewei comments: QMP is a JSON-based protocol which allows
	// applications to control a QEMU instance, refers to
